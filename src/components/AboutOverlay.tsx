/**
 * AboutOverlay — full-screen "About CarFM" page (opened from the menu
...
// Stuart's personal message — source: reference/CarFM About.rtf (proofread,
...
const VERSION_HISTORY: { v: string; detail: string }[] = [
  ...
  { v: 'V2.2', detail: 'Siri voice control (iOS). Say "Hey Siri, tune CarFM" — Siri asks what — then a frequency (7.150 MHz / 7150 kHz / 7151.5), a station ("Radio Caroline"), or a band ("40m ham", "CB"). It tunes with the right demodulator + step, honouring any spoken mode. When a name matches several bookmarks (e.g. "Radio 5") Siri reads the frequencies and you pick by voice; "China Radio at 11 MHz" narrows the list. Also "change CarFM mode" → AM / SAM / synchronous AM / LSB / lower side band / …, and "set CarFM step rate" → 100 Hz, 9 kHz, … It runs in the background while CarFM is playing, so it works over headphones / CarPlay / the lock screen without unlocking. (Tuning is a two-step ask-and-answer — Apple only allows a value inside a one-shot Siri phrase for fixed lists. Android’s in-car answer stays the Android Auto Bookmarks/Band-Plan browse — Google Assistant needs Play Store publishing.)' },
  { v: 'V2.2.1', detail: 'In-car fix: a Siri voice command interrupts the car audio session, which paused and disconnected CarFM — it then sat dead until you pressed Play. It now auto-resumes (reconnects on the new frequency) the moment Siri finishes, with no manual Play. A genuine takeover by another app (e.g. a Mac grabbing your AirPods) still waits for Play, as before.' },
  ...
  { v: 'V5.2.0', detail: 'Deep linking (early feature, still being rolled out). A carfm:// link — and a QR code from an UberSDR instance — can open CarFM straight onto that instance, optionally at a set frequency and mode. The link/QR side is still being built on the UberSDR end, so not every instance offers a link yet. The share button now also includes an “Open in CarFM” app link alongside the web link. Opening a link no longer bounces back to your default instance.' },
 ...
 *  that makes CarFM look BETTER, not worse: they are principles and legal facts, not
...
const V9_CHANGES: string[] = [
  'CarFM on your wrist. A real Apple Watch app — not a remote control with a few buttons, but the waterfall itself, live on the watch, drawn from the same data and the same palette as the phone. Turn the Digital Crown to tune, tap the frequency to type one, press and hold for the menu (demodulator, tuning step, zoom, servers).',
  'It works with the iPhone LOCKED IN YOUR POCKET, which is the whole point. It will even start the phone for you: open the watch app with CarFM closed and the phone wakes up in the background, connects to your default receiver, and the waterfall arrives on your wrist without the phone screen ever coming on.',
...
const V8_0_1_CHANGES: string[] = [
  'Fixed: your saved favourites could start connecting as a VibeServer instead of the UberSDR receiver they actually are. CarFM works out what kind of server it is talking to by looking at its web page — and it was treating the word “CarFM” as evidence of a VibeServer. But UberSDR receivers carry a “open in CarFM” link of their own, so a perfectly ordinary UberSDR looked like one of ours. Worse, the wrong answer was then saved back onto the favourite. CarFM now looks for a marker only a real VibeServer carries, and repairs any favourites that were mislabelled.',
];

const V8_CHANGES: string[] = [
  'VibeServer — turn an Android phone with an RTL-SDR into a receiver anyone on your network can use, from a web browser or from CarFM on another phone. Point a browser at the phone’s address and the full client is there: no install, no app.',
  ...
  'PIN protected by challenge-response, so the PIN itself never crosses the network — or run it open on a LAN you trust. You can also switch the web client off entirely, so only the CarFM app can connect and nobody stumbles into your receiver from a URL.',
  ...
  'The receiver names the stations it can hear. When a station announces itself over RDS, CarFM remembers it against the frequency, so the search bar fills itself in with what this aerial actually receives rather than a schedule of what merely exists. The station’s PI code keeps it honest — a different broadcaster on the same frequency is spotted immediately — and the name is reconstructed by majority vote across repetitions, so it can recover a name that no single transmission delivered cleanly.',
  ...
  'The RTL-TCP box becomes CUSTOM SERVER: type any address and CarFM works out what is listening — VibeServer, OpenWebRX, KiwiSDR, UberSDR, FM-DX, rtl_tcp or SpyServer — so one box reaches every backend. Local hardware is now RTL-SDR, with Listen and Use as server side by side.',
  'Station logos and country flags now actually appear — on every backend, and on AM and shortwave too, not just FM. Where the country genuinely cannot be known (a station arriving on sporadic-E, say) CarFM declines to show a flag rather than showing your own country’s.',
...
const V7_CHANGES: string[] = [
  ...
  '7.0.1 (iOS) — Scanning a QR code or opening a carfm:// link with the app fully closed now opens the correct receiver instead of your default instance.',
  'FM-DX Webserver support: CarFM now connects to the worldwide network of FM-DX Webserver tuners (from servers.fmdx.org) — real remote FM broadcast tuners, shared live with other listeners. Pick one from the new FM-DX directory (or save it as a favourite).',
...
const V6_1_CHANGES: string[] = [
  'Auto-discovery of RTL-TCP servers on your network: CarFM now finds rtl_tcp servers on your Wi-Fi automatically (via Bonjour/mDNS) and lists them under a new “Discovered” section — no need to type an IP address. Tap to connect, or tap the star to save it. Works on both iOS and Android.',
  'Share your RTL-SDR over the network (Android): plug an RTL-SDR into an Android phone and you can now run it as an RTL-TCP server — other devices (an iPhone running CarFM, SDR#, etc.) connect to it over Wi-Fi and use the dongle remotely. Great for putting the dongle somewhere with a good antenna, or on a phone that’s always on. Includes an optional bandwidth cap if the connection struggles, an editable name shown to other devices, and a live status notification.',
...
const V6_CHANGES: string[] = [
  'iOS 27 support: CarFM is rebuilt on React Native’s New Architecture, required for the iOS 27 / Xcode 27 toolchain (Apple no longer accepts the older one). This is a big under-the-hood modernisation; everything you already use works the same.',
  ...
  'Plug-and-go on Android: plugging an RTL-SDR into the phone and choosing “Open in CarFM” now takes you straight to Local Hardware instead of your default instance.',
  'Background audio on aggressive phones (Android): some phones (e.g. certain Motorola models) restrict apps in the background by default, which broke up local audio when the screen was off. CarFM now detects this and shows you exactly how to allow background usage so playback holds up.',
  'Swiping CarFM out of the recent-apps list now fully shuts it down on Android — it no longer lingers as a media notification in the shade (GitHub #6). The Stop button in the media notification also closes it.',
...
const V5_1_CHANGES: string[] = [
  ...
  'Deep linking (early feature, still rolling out): a carfm:// link or QR code from an UberSDR instance opens CarFM straight onto that receiver, optionally at a set frequency and mode. The link/QR side is still being built on the UberSDR end, so not every instance offers a link yet. Sharing a tuned station now also includes an “Open in CarFM” app link (new in 5.2.0)',
...
const V5_CHANGES: string[] = [
  'SDR++ Brown, FFTW and VOLK are gone — the on-device radio (Local Hardware + RTL-TCP) now runs on VibeDSP, CarFM’s own clean-room DSP engine, written from scratch and free of any GPL third-party DSP',
...
const V4_CHANGES: string[] = [
  'Local SDR hardware (Android): plug an RTL-SDR into your phone over USB and pick “Local Hardware” — CarFM runs the radio on-device with a bundled DSP core, with the same waterfall, drum, audio and decoders as any remote server, plus a hardware-control submenu (gain, PPM, bias-T, AGC, sample rate, direct sampling)',
...
const V3_CHANGES: string[] = [
  ...
  'KiwiSDR support: waterfall, audio, tuning, real dBm S-meter, server-side zoom and a live connection meter — same custom interface as every other backend. (Owners choose who connects: some refuse apps or block broadcast bands, so a server may refuse or drop you — that\'s the owner\'s restriction, not CarFM.)',
    ...
    >
      <SafeAreaView style={styles.root} edges={['top']}>
        <View style={styles.bar}>
          ...
          <Text style={styles.title}>About CarFM</Text>
          <View style={{ width: 50 }} />
        </View>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
          <View style={styles.heroRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.appName}>CarFM V7</Text>
          ...
          <Text style={styles.body}>
            CarFM stands on the work of other open projects. Thank you to all of them.
          </Text>
          {CREDITS.map((c) => (
          ...
          <Text style={styles.body}>
            CarFM is free software, released under the GNU General Public License
          ...
          <Text style={styles.body}>
            If you bought CarFM from an app store, that nominal price simply covers the
          ...
          <TouchableOpacity onPress={() => Linking.openURL('https://github.com/Stuey3D/CarFM')}>
            <Text style={styles.link}>Source code: github.com/Stuey3D/CarFM</Text>
          </TouchableOpacity>
          ...
          <Text style={styles.body}>
            CarFM collects no personal data — no analytics, ads, or tracking.
          ...
          <TouchableOpacity onPress={() => Linking.openURL('https://github.com/Stuey3D/CarFM/blob/main/PRIVACY.md')}>
