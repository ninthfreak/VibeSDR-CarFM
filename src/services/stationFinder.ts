...
import { callsignBase } from './piCallsign';
import { identifyFromSource } from './stationIdentify';
import type { NearbyStation, StationIdentity } from './stationTypes';
 ...
 *
 * Every rule lives in stationIdentify.identifyFromSource, which is pure and is
 * what the Rust port is diffed against. This supplies the two database reads and
 * fires the enrichment side effect, which is not part of identifying anything.
 */
export async function identifyByPi(
  pi: number, psText?: string, freqMhz?: number,
): Promise<StationIdentity> {
  const id = await identifyFromSource(pi, psText, freqMhz, {
    forCallsignBase: stationsForCallsignBase,
    atFrequency: stationsAtFrequency,
  });
  if (id.station) {
    ...
    void noteEncountered({
      base: callsignBase(id.callsign ?? id.station.callsignBase),
      callsign: id.callsign ?? id.station.callsignBase,
      homepage: null,
      name: id.station.callsign,
      freqMhz: id.station.frequencyMhz,
    });
  }
  return id;
}
